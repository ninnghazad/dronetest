...
minetest.register_chatcommand("dronetest", {
	...
	func = function(name, param)
		...
		minetest.show_formspec(name, "dronetest:menu",dronetest.get_menu_formspec())
	end,
})
...
minetest.register_chatcommand("dronetest:info", {
	...
	func = function(name, param)
		...
		local info = ""
		info = info.."# active systems: "..count(dronetest.active_systems).."\n# active drones: "..count(dronetest.drones).."\n"
		info = info.."# output buffers: "..count(dronetest.console_histories).."\n"
		local num_events = 0
		for _,v in pairs(dronetest.active_systems) do
			num_events = num_events + count(v.events)
		end
		info = info.."# total events in all queues: "..num_events.."\n"
		
		local num_buffsize = 0
		for _,v in pairs(dronetest.console_histories) do
